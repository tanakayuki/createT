...
*/
if(isset($_GET['clear'])){
	unset($_SESSION['sql_data2']);
}

if(!isset($_SESSION['sql_data2'])){
			/*************************************
			*
			*$sql　   表示用
			*$sql_all　カウント用
			...
			**************************************/
			...
			$_SESSION['sql_data2'] = array('sql2' => $sql,'sql_all2' => $sql_all);
}else{
	$sql = $_SESSION['sql_data2']['sql2'];
	$sql_all = $_SESSION['sql_data2']['sql_all2'];
		/*************************************
			*
...
if(!$list = $DAO -> SQL($sql,"")){
	...
	<?php
		...
		$DAO->next_page($page_point,$idx,$_SESSION['sql_data2']['sql2']);
	?>
	</ul>
	</div>
<?php }?>
